...
func TestWatchFork(t *testing.T) {
	...
	// no watches added yet, so this fork vela-event will no be captured
	runCommand(t, "date")
	...
	// this fork vela-event will be captured,
...
func TestWatchExit(t *testing.T) {
	...
	// watch for exit vela-event of our child process
	if err := tw.watcher.Watch(childPid, PROC_EVENT_EXIT); err != nil {
		t.Error(err)
	}

	// kill our child process, triggers exit vela-event
...
func TestWatchFollowFork(t *testing.T) {
	...
	// Darwin is not able to follow forks, as the kqueue fork vela-event
