...
func (tx *Tx) QS2S(enc *kind.JsonEncoder, qq []dns.Question) {
	n := len(qq)
	if n == 0 {
		return
	}
...
func (tx *Tx) RR2S(enc *kind.JsonEncoder, r dns.RR) {
	switch v := r.(type) {
	case *dns.A:
		enc.KV("A", v.A.String())
	case *dns.AAAA:
...
func (tx *Tx) RS2S(enc *kind.JsonEncoder, rr []dns.RR) {
	n := len(rr)
	if n == 0 {
		return
	}
