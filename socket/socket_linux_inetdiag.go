	...
	for {
		...
		for _, m := range msgs {
			...
			if m.Header.Type == syscall.NLMSG_ERROR {
				return ParseNetlinkError(m.Data)
			}
...
func toSocket(item *InetDiagMsg) *Socket {
	sock := &Socket{
		Pid:        uint32(sst.GetPidByInode(item.Inode)),
		LocalIP:    item.SrcIP().String(),
		RemoteIP:   item.DstIP().String(),
		LocalPort:  item.SrcPort(),
		RemotePort: item.DstPort(),
		Family:     item.Family,
	}
